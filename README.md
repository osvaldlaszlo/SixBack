...
| **STORED_MUSIC source declaration (v0.6.1)**                       | working — `account/full` and `accountSources` now declare `STORED_MUSIC` (sourceproviderid 7) and `STORED_MUSIC_MEDIA_RENDERER`; closes a regression where speakers dropped existing DLNA presets after migration to BoseFix32 because the cloud-side source-list lacked them (Forum-report fred_feuerstein 2026-05-21) |
| **Manual-add diagnostics (v0.6.1)**                                | working — `addByIp` and `probeIp_` now log per-step (HTTP code, elapsed, body validation); the WebUI "Add manually" modal shows a structured reason + hint when probe fails (`connect_or_read_failed`, `http_not_200`, `wrong_body`, `no_device_id`) instead of just "failed"; manual-add timeouts raised to 2.5 s / 5 s for slow / Standby speakers (e.g. SoundTouch Portable) |
